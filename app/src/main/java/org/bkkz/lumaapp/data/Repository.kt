...
class Repository(
    ...
    suspend fun getLocalUserTaskByDate(datePrefix: String): List<UserTaskEntity> {
        return withContext(Dispatchers.IO) {
            userTaskDao.getUserTasksByDatePrefix(datePrefix)
        }
    }
        ...
        withContext(Dispatchers.IO) {
            try {
                ...
                if (response == null) {
                    ...
                    if (taskList != null) {
                        val taskListIds = taskList.map { it.id }
                        val existingLocalId = getLocalUserTaskByDate(date).map { it.id }
                        val idsToDelete = existingLocalId.filter { it !in taskListIds }
                        for (id in idsToDelete) {
                            deleteLocalUserTaskById(id)
                        }
    ...
    suspend fun editTask(taskId: String, editTaskRequest: EditTaskRequest): ApiResult<Any> =
        withContext(
            Dispatchers.IO
        ) {
            try {
    ...
    suspend fun getReportHistory(formType: String): ApiResult<ApiResponse<ReportHistory>> =
        withContext(
            Dispatchers.IO
        ) {
            try {
    ...
    suspend fun insertGoogleCalendarEvent(calendarEventRequest: CalendarEventRequest): ApiResult<String?> = withContext(Dispatchers.IO) {
        try {
            ...
            if(response.isSuccessful){
                ...
                ApiResult.Error(Exception("Cannot create google calendar event"))
            }
        }catch (e: Exception){
            ApiResult.Error(Exception(e.message))
        }
